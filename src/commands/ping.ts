...
export class UserCommand extends Command {
	...
	private async sendPing(interactionOrMessage: Message | Command.ChatInputCommandInteraction | Command.ContextMenuCommandInteraction) {
		if (interactionOrMessage instanceof Message) {
			const sentMessage = interactionOrMessage.channel?.isSendable() && (await interactionOrMessage.channel.send({ content: 'Ping?' }));
			if (!sentMessage) return;

			const content = `Pong! Bot Latency ${Math.round(this.container.client.ws.ping)}ms. API Latency ${
				sentMessage.createdTimestamp - interactionOrMessage.createdTimestamp
			}ms.`;

			return sentMessage.edit({ content });
		}

		// For interactions, use deferReply and then editReply
		await interactionOrMessage.deferReply();

		const content = `Pong! Bot Latency ${Math.round(this.container.client.ws.ping)}ms. API Latency ${
			Date.now() - interactionOrMessage.createdTimestamp
		}ms.`;
